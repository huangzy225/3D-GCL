...
__global__ void cal_corr_cuda_kernel(
    ...
    const torch::PackedTensorAccessor32<scalar_t,3,torch::RestrictPtrTraits> vis_sym_mask_1, 
    const torch::PackedTensorAccessor32<scalar_t,1,torch::RestrictPtrTraits> symmetric_table, 
    ...
    const int image_width)
{
    ...
    if (face_id != -1){                                             // not background
        if(abs(visible_faces_2[batch_id][face_id]-1) < 0.001){
            corr_mask[batch_id][x][y] = 0.;
        }
        int vert_ia = int(face[face_id][0]);   
        int vert_ib = int(face[face_id][1]);    
        int vert_ic = int(face[face_id][2]); 
        ...
        auto tri_2a0 = verts_2[batch_id][vert_ia][0];
        auto tri_2a1 = verts_2[batch_id][vert_ia][1];

        auto tri_2b0 = verts_2[batch_id][vert_ib][0];
        auto tri_2b1 = verts_2[batch_id][vert_ib][1];

        auto tri_2c0 = verts_2[batch_id][vert_ic][0];
        auto tri_2c1 = verts_2[batch_id][vert_ic][1];


        // for invisible pixels whose corresponding symmetric verteices are visible, rewrite flow map and corr_mask value
        if(abs(visible_faces_2[batch_id][face_id]-1) >= 0.1 && vis_sym_mask_1[batch_id][x][y] > 0.1){
            const auto sym_vert_ia = int(symmetric_table[face[face_id][0]]);   
            const auto sym_vert_ib = int(symmetric_table[face[face_id][1]]);    
            const auto sym_vert_ic = int(symmetric_table[face[face_id][2]]); 

            // tri_1/2: 3x2      
            tri_2a0 = verts_2[batch_id][sym_vert_ia][0];
            tri_2a1 = verts_2[batch_id][sym_vert_ia][1];

            tri_2b0 = verts_2[batch_id][sym_vert_ib][0];
            tri_2b1 = verts_2[batch_id][sym_vert_ib][1];

            tri_2c0 = verts_2[batch_id][sym_vert_ic][0];
            tri_2c1 = verts_2[batch_id][sym_vert_ic][1];
...
std::vector<torch::Tensor> cal_corr_cuda(torch::Tensor vis_1, 
                                         ...
                                         torch::Tensor vis_sym_mask_1,
                                         torch::Tensor symmetric_table
                                         ){
    const int warp_size = 32;
    const dim3 blocks(warp_size, warp_size, 1);     // determine size of grids, blocks, threads
    ...
    AT_DISPATCH_FLOATING_TYPES(vis_1.type(), "cal_corr_cuda", ([&] {
        cal_corr_cuda_kernel<scalar_t><<<grids, blocks>>>(
            ...
            vis_sym_mask_1.packed_accessor32<scalar_t,3,torch::RestrictPtrTraits>(),
            symmetric_table.packed_accessor32<scalar_t,1,torch::RestrictPtrTraits>(),
