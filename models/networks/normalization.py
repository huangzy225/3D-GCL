...
class SPADEStyleGAN(nn.Module):
    def __init__(self, config_text, norm_nc, label_nc, PONO=False):
        ...
        if self.pad_type != 'zero':
            ...
            self.mlp_gamma = nn.Conv2d(nhidden, norm_nc, kernel_size=ks, padding=pw)
            self.mlp_beta = nn.Conv2d(nhidden, norm_nc, kernel_size=ks, padding=pw)
            # self.mlp_shared = Conv2dLayer(label_nc, nhidden, kernel_size=ks)
            # self.mlp_gamma = Conv2dLayer(nhidden, norm_nc, kernel_size=ks)
            # self.mlp_beta = Conv2dLayer(nhidden, norm_nc, kernel_size=ks)
