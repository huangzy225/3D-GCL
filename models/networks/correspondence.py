...
class GlobalCorrespondence(BaseNetwork):
    def __init__(self, opt):
        self.opt = opt
        super().__init__()

        self.content_encoder = FeatureEncoder_v2(opt, input_nc=3)
        self.refernce_encoder = FeatureEncoder_v2(opt, input_nc=3+3)
        ...
        # self.phi_4 = nn.Conv2d(in_channels=feature_channel*2, out_channels=feature_channel, kernel_size=1, stride=1, padding=0)
        # self.theta_4 = nn.Conv2d(in_channels=feature_channel*2, out_channels=feature_channel, kernel_size=1, stride=1, padding=0)

        # self.GACRM = GACRMBlock(opt)  # 128
        # self.GACRM2 = GACRMBlock(opt)  # 256
        # self.GACRM3 = GACRMBlock(opt)  # 512
    ...
    def forward(self, data_unpack):
        ...
        #     cont_features[i] = util.feature_normalize(cont_features[i])
        #     ref_features[i] = util.feature_normalize(ref_features[i])

        theta = []
        phi = []
        """512x512"""
        theta.append(match_kernel_and_pono_c(self.theta_1(cont_features[0]), self.opt.match_kernel, self.opt.PONO_C))
        phi.append(match_kernel_and_pono_c(self.phi_1(ref_features[0]), self.opt.match_kernel, self.opt.PONO_C))
          
        # theta.append(match_kernel_and_pono_c(self.theta_2(cont_features[1]), self.opt.match_kernel, self.opt.PONO_C))
        # phi.append(match_kernel_and_pono_c(self.phi_2(ref_features[1]), self.opt.match_kernel, self.opt.PONO_C))
        
        # theta.append(match_kernel_and_pono_c(self.theta_3(cont_features[2]), self.opt.match_kernel, self.opt.PONO_C))
        # phi.append(match_kernel_and_pono_c(self.phi_3(ref_features[2]), self.opt.match_kernel, self.opt.PONO_C))

        ref = cur_cloth_ref
        self.cloth_mask = data_unpack['cloth_mask']
        ys = []
        m = None
        m, y, y_cycle = self.multi_scale_warping(theta[0], phi[0], ref, 0, pre=m)
        ys.append(y)
        ...
        # ys.append(y)
        # from util.util_func import visualize_img
        # visualize_img([y, y_cycle])

        corr_out['warp_out'] = ys
        corr_out['y_cycle'] = y_cycle
        corr_out['corr_m'] = m
