...
def save_network(net, label, epoch, opt, rank=None):
...
def load_network(net, label, epoch, opt, rank=None):
    save_filename = '%s_net_%s.pth' % (epoch, label)
    save_dir = os.path.join(opt.checkpoints_dir, opt.name)
    save_path = os.path.join(save_dir, save_filename)
    if not os.path.exists(save_path) and (rank is None or rank == 0):
...
def print_current_errors(opt, writer, epoch, i, num, errors, t, start_time, global_step):
    message = '(epoch: %d, iters: %d, finish: %.2f%%, time: %.3f) ' % (epoch, global_step, (i/num)*100.0, t)
...
def log_errors(writer, errors, start_time, global_step):
    for k, v in errors.items():
        v = v.mean().float()
        walltime = time.time() - start_time
        writer.add_scalar(k, v, global_step=global_step, walltime=walltime)
    writer.flush()